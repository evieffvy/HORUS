...
async function exportPDF(cve: CVEItem, summary: string) {
  ...
  if (summary) {
    ...
    doc.text('AI Summary (Thai)', 15, y2 + 5)
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(148, 163, 184)
...
export default function CVEModal({ cve, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch('/api/summarize', {
      ...
      .catch(() => setSummary('ไม่สามารถสร้างสรุปได้ในขณะนี้'))
  ...
  return (
    ...
    >
      <div
        className="card w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl border-border-bright"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-start justify-between p-5 border-b border-border">
          <div>
            <h2 className="font-mono text-blue-400 font-semibold text-lg">{cve.id}</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              เผยแพร่: {fmtDate(cve.publishedDate)}
            </p>
          </div>
            ...
            <div className="text-right">
              <div
                className={`text-2xl font-bold font-mono ${
                  { CRITICAL: 'text-red-400', HIGH: 'text-orange-400', MEDIUM: 'text-yellow-400', LOW: 'text-green-400', NONE: 'text-gray-400', UNKNOWN: 'text-gray-500' }[cve.severity]
                }`}
              >
        ...
        <div className="h-1 w-full bg-border">
          <div
            ...
            style={{ width: `${((cve.cvssScore ?? 0) / 10) * 100}%` }}
          />
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              <div key={label} className="bg-[#0a1929] rounded-lg px-3 py-2">
          ...
          {/* Description */}
          <div>
            <h3 className="text-xs font-semibold text-slate-400 mb-2">รายละเอียด</h3>
          ...
          {cve.references.length > 0 && (
            <div>
              <h3 className="text-xs font-semibold text-slate-400 mb-2">References</h3>
                ...
                {cve.references.map((ref) => (
                  <li key={ref}>
                    <a
                      ...
                      className="text-xs text-blue-400/80 hover:text-blue-300 transition-colors truncate block"
                    >
                      {ref}
                    </a>
                  </li>
